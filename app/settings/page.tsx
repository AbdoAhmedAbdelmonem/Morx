...
export default function SettingsPage() {
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Note: Express.js backend only supports updating first_name and last_name
      // Password update and other profile fields are not supported
      const { API_BASE_URL, API_ENDPOINTS, getAuthHeaders } = await import('@/lib/api-config')

      const res = await fetch(`${API_BASE_URL}${API_ENDPOINTS.USERS.UPDATE_NAME(user.user_id)}`, {
        method: 'PUT',
      ...
      if (res.ok) {
        ...
        safeLocalStorage.setItem('student_session', JSON.stringify(updatedUser))
