...
export default function TeamsPage() {
  ...
  const fetchTeams = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}${API_ENDPOINTS.TEAMS.GET_ALL}`, {
        method: 'GET',
  ...
  const handleCreateTeam = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Add the new team to the list immediately
        const newTeam = {
          team_id: result.teamId,
          name: result.team?.team_name || newTeamName,
          url: result.team?.team_url,
          description: '',
          members: 1,
          projects: 0,
          role: 'admin',
  ...
  const handleUpdateTeam = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}${API_ENDPOINTS.TEAMS.UPDATE(teamToEdit.team_id)}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
        body: JSON.stringify({ team_name: editTeamName })
      });
      ...
      if (res.ok) {
        ...
        setTeams(prev => prev.map(t => 
          t.team_id === teamToEdit.team_id 
            ? { ...t, name: editTeamName, description: editTeamDescription }
            : t
        ));
  ...
  const handleDeleteTeam = async (teamUrl: string, teamName: string, projectCount: number) => {
    ...
    if (team) {
      setTeamToDelete({ url: teamUrl, name: teamName, projectCount, teamId: team.team_id } as any);
      setIsDeleteDialogOpen(true);
    }
  }

  const confirmDeleteTeam = async () => {
    if (!teamToDelete || deleteConfirmText !== teamToDelete.name || !deleteConfirmChecked) return;
    
    try {
      const team = teams.find(t => t.url === teamToDelete.url);
      if (!team) return;
      ...
      if (res.ok) {
        ...
        setTeams(prev => prev.filter(t => t.url !== teamToDelete.url));
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5 lg:gap-6">
              {teams.map((team: any) => (
                <Card 
                  ...
                  onClick={() => router.push(`/teams/${team.url}`)}
                >
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem 
                              onClick={(e) => {
                                e.stopPropagation()
                                handleDeleteTeam(team.url, team.name, team.projects || 0)
                              }} 
                  ...
                  <CardContent className="pt-0">
                    <div className="flex items-center justify-between">
                      <Badge variant="secondary" className="text-xs">
                        {team.projects || 0} project{team.projects !== 1 ? 's' : ''}
                      </Badge>
