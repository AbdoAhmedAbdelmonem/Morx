...
export default function TeamReportsPage() {
  ...
  const fetchReportData = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}${API_ENDPOINTS.TEAMS.GET_REPORT(teamUrl)}?period=${selectedPeriod}&project=${selectedProject}`, {
        method: 'GET',
        headers: getAuthHeaders(),
      })
      const result = await res.json()
      ...
      if (res.ok) {
        // Set team from report response if not already set
        if (result.team) {
          setTeam(result.team)
        }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-6">
            ...
            <ChartCard
              ...
              data={reportData.project_stats.map((proj: any) => ({
                name: (proj.project_name || 'Unnamed')?.length > 15 ? (proj.project_name || 'Unnamed').substring(0, 15) + '...' : (proj.project_name || 'Unnamed'),
                completed: proj.completed || 0,
                'in progress': proj.in_progress || 0,
                todo: proj.todo || 0
              }))}
              type="bar"
              dataKey="completed"
            ...
            <ChartCard
              ...
              data={reportData.member_performance.map((member: any) => ({
                ...
                'Completion Rate': member.completion_rate || 0,
                'Tasks': member.assigned_tasks || 0
              }))}
              type="line"
