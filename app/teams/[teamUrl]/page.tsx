...
export default function TeamDetailPage() {
  ...
  const [projectToDelete, setProjectToDelete] = useState<{ id: number; url: string; name: string; taskCount: number } | null>(null)
  ...
  const fetchTeam = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}${API_ENDPOINTS.TEAMS.GET_BY_URL(teamUrl)}`, {
        method: 'GET',
  ...
  const fetchProjects = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/projects?team_id=${team.team_id}`, {
        method: 'GET',
        headers: getAuthHeaders(),
      })
      const result = await res.json()
  ...
  const fetchTeamMembers = async () => {
    ...
    // Team members are already included in team data from Express backend
    if (team.members) {
  ...
  const handleAddMember = async () => {
    if (!newMemberEmail.trim() || !user?.user_id || !team) return
    
    try {
      const res = await fetch(`${API_BASE_URL}${API_ENDPOINTS.TEAMS.ADD_MEMBER(team.team_id)}`, {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({
      ...
      if (res.ok) {
        ...
        fetchTeam() // Refresh team data to get updated members
  ...
  const handleRemoveMember = async (memberId: number, memberName: string) => {
    ...
    if (!user?.user_id || !team) return
    
    try {
      const res = await fetch(`${API_BASE_URL}${API_ENDPOINTS.TEAMS.REMOVE_MEMBER(team.team_id, memberId)}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      })
  ...
  const handleChangeRole = async (memberId: number, newRole: string, memberName: string) => {
    ...
    // TODO: Implement role change endpoint in Express backend
  ...
  const handleCreateProject = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}${API_ENDPOINTS.PROJECTS.CREATE}`, {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({
  ...
  const handleDeleteProject = async (projectId: number, projectUrl: string, projectName: string, taskCount: number) => {
    setProjectToDelete({ id: projectId, url: projectUrl, name: projectName, taskCount })
  ...
  const confirmDeleteProject = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}${API_ENDPOINTS.PROJECTS.DELETE(projectToDelete.id)}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      })
          ...
          <div className="mb-6 sm:mb-8">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="flex items-center gap-2 w-full lg:w-auto flex-wrap">
                ...
                {/* Members/Settings Button - visible to all, actions limited for non-admins */}
                <Button
                  variant="outline"
          ...
          {projects.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 sm:py-16 md:py-24 px-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                {isOwnerOrAdmin && (
                  <Button size="lg" variant="outline" onClick={() => setIsAddMemberOpen(true)} className="h-11 px-6">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem 
                                onClick={(e) => {
                                  e.stopPropagation()
                                  handleDeleteProject(project.project_id, project.project_url, project.project_name, project.task_count || 0)
                                }} 
                                className="text-destructive"
                              >
