...
export function NotificationPanel({ userId }: NotificationPanelProps) {
  ...
  const fetchNotifications = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}${API_ENDPOINTS.NOTIFICATIONS.GET_ALL}`, {
      ...
      // Express.js backend returns array directly, not { success, data }
      if (Array.isArray(result)) {
        setNotifications(result)
        setUnreadCount(result.filter((n: Notification) => n.is_read === 0).length || 0)
      }
    } catch (error) {
  ...
  const markAsRead = async (notificationId: number) => {
    try {
      const res = await fetch(`${API_BASE_URL}${API_ENDPOINTS.NOTIFICATIONS.MARK_READ(notificationId)}`, {
        method: 'PUT',
        headers: getAuthHeaders()
      })
  ...
  // Note: Express.js backend doesn't have a mark-all-as-read endpoint
